...
export async function GET() {
  ...
  return NextResponse.json(await managementService.listDesks(auth.user));
}
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json(await managementService.createDesk(auth.user, parsed.data, getRequestContext(request).ipAddress), {
    status: 201
  });
}
