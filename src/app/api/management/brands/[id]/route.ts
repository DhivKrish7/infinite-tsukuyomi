...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const result = await managementService.updateBrand(auth.user, (await params).id, parsed.data, getRequestContext(request).ipAddress);
  return result ? NextResponse.json(result) : NextResponse.json({ error: "Brand not found" }, { status: 404 });
}
