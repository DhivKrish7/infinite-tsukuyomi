...
export async function PATCH(request: NextRequest, { params }: Params) {
  ...
  const result = await managementService.updateIpRestriction(auth.user, (await params).id, parsed.data, getRequestContext(request).ipAddress);
  return result ? NextResponse.json(result) : NextResponse.json({ error: "IP restriction not found" }, { status: 404 });
}
